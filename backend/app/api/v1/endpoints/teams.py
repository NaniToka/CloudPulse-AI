...
from app.schemas.tenant import TeamCreate, TeamResponse, TeamUpdate
...
@router.post(
    "", response_model=TeamResponse, status_code=status.HTTP_201_CREATED, summary="Create Team"
)
...
async def update_team(
    ...
    team = await service.update_team(
        db, team_id, name=payload.name, description=payload.description
    )
    if not team:
        raise HTTPException(status_code=404, detail="Team not found")
    return TeamResponse.model_validate(team)
