...
from app.services.runbook_service import runbook_service, RunbookService
...
@router.post("/generate", response_model=RunbookResponse, status_code=status.HTTP_201_CREATED, summary="Generate AI remediation runbook")
...
async def list_runbooks(
    ...
    status_filter: Optional[str] = Query(None, alias="status", description="Filter by status (Draft, Approved, Completed)"),
...
@router.get("/{runbook_id}", response_model=RunbookResponse, summary="Get runbook details & automation steps")
...
@router.post("/{runbook_id}/approve", response_model=RunbookResponse, summary="Approve runbook for execution")
...
@router.post("/{runbook_id}/execute", response_model=RunbookExecutionResponse, summary="Execute runbook automation steps")
async def execute_runbook(
    ...
    if not execution:
        raise HTTPException(status_code=404, detail=f"Runbook '{runbook_id}' not found or not approved.")
