...
class Trace(UUIDMixin, TimestampMixin, Base):
    ...
    name: Mapped[str] = mapped_column(String(500), nullable=False)  # e.g., GET /api/v1/users/checkout
...
class Span(UUIDMixin, Base):
    ...
    trace_id: Mapped[str] = mapped_column(String(128), ForeignKey("traces.trace_id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    span_kind: Mapped[str] = mapped_column(String(50), nullable=False, default="SERVER")  # SERVER, CLIENT, INTERNAL
