...
class Workflow(UUIDMixin, TimestampMixin, Base):
    ...
    status: Mapped[str] = mapped_column(String(50), nullable=False, default="active")  # active | paused | draft
    trigger_type: Mapped[str] = mapped_column(String(100), nullable=False, default="manual")  # incident_created | alert_fired | cpu_threshold | cron | manual | webhook
    trigger_config: Mapped[dict] = mapped_column(JSON, default=dict)  # threshold values, cron expression, webhook secrets

    # Visual Canvas DAG representation
    nodes: Mapped[list] = mapped_column(JSON, default=list)  # list of { id, type, label, config, position: {x, y} }
    edges: Mapped[list] = mapped_column(JSON, default=list)  # list of { id, source, target, condition?: str }

    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    tags: Mapped[list] = mapped_column(JSON, default=list)  # ["k8s", "remediation", "incident", "slack"]
...
class WorkflowExecution(UUIDMixin, TimestampMixin, Base):
    ...
    status: Mapped[str] = mapped_column(String(50), nullable=False, default="running")  # running | completed | failed | awaiting_approval | rolled_back
...
class WorkflowStepLog(UUIDMixin, TimestampMixin, Base):
    ...
    action_type: Mapped[str] = mapped_column(String(100), nullable=False)  # k8s_restart | slack_notify | gemini_analyze | etc.
    status: Mapped[str] = mapped_column(String(50), nullable=False, default="completed")  # completed | failed | skipped | awaiting_approval
    ...
    # Relationships
    execution: Mapped["WorkflowExecution"] = relationship("WorkflowExecution", back_populates="step_logs")
...
class WorkflowApproval(UUIDMixin, TimestampMixin, Base):
    ...
    approver_role: Mapped[str] = mapped_column(String(100), nullable=False, default="admin")  # admin | sre | devops
    ...
    # Relationships
    execution: Mapped["WorkflowExecution"] = relationship("WorkflowExecution", back_populates="approvals")
...
class WorkflowTemplate(UUIDMixin, TimestampMixin, Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    category: Mapped[str] = mapped_column(String(100), nullable=False, default="Kubernetes")  # Kubernetes | Security | Incident | Cost
