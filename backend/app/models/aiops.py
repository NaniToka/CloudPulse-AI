...
class AIOpsAgent(UUIDMixin, TimestampMixin, Base):
    ...
    agent_name: Mapped[str] = mapped_column(
        String(255), nullable=False, default="CloudPulse Autonomous Core"
    )
    status: Mapped[str] = mapped_column(
        String(50), nullable=False, default="Autonomous"
    )  # Active, Autonomous, Paused
    current_phase: Mapped[str] = mapped_column(
        String(50), nullable=False, default="Observe"
    )  # Observe, Detect, Analyze, Plan, Recommend, Verify
    health_status: Mapped[str] = mapped_column(
        String(50), nullable=False, default="Healthy"
    )  # Healthy, Degraded, Anomalous
    ...
    recommendations: Mapped[list["AgentRecommendation"]] = relationship(
        "AgentRecommendation", back_populates="agent", cascade="all, delete-orphan"
    )
...
class AgentTask(UUIDMixin, Base):
    ...
    agent_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("aiops_agents.id", ondelete="CASCADE"), nullable=False
    )
    task_name: Mapped[str] = mapped_column(String(255), nullable=False)
    target_system: Mapped[str] = mapped_column(
        String(100), nullable=False
    )  # Metrics, Logs, Traces, Security, Cost
    status: Mapped[str] = mapped_column(
...
class AgentRecommendation(UUIDMixin, TimestampMixin, Base):
    ...
    category: Mapped[str] = mapped_column(
        String(100), nullable=False, default="Anomaly_Detection"
    )  # Anomaly_Detection, Root_Cause, Performance, Capacity_Planning, Risk_Prediction, Cost_Optimization, Security_Correlation
    ...
    executions: Mapped[list["AgentExecution"]] = relationship(
        "AgentExecution", back_populates="recommendation", cascade="all, delete-orphan"
    )
...
class AgentExecution(UUIDMixin, Base):
    ...
    approved_by: Mapped[str] = mapped_column(
        String(255), nullable=False, default="AIOps Autonomous Controller"
    )
    status: Mapped[str] = mapped_column(String(50), default="Completed")
    execution_logs: Mapped[list] = mapped_column(JSON, nullable=True, default=list)
    executed_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)

    recommendation: Mapped["AgentRecommendation"] = relationship(
        "AgentRecommendation", back_populates="executions"
    )
