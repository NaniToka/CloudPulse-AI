...
class Runbook(UUIDMixin, TimestampMixin, Base):
    ...
    severity: Mapped[str] = mapped_column(String(50), nullable=False, default="P1")  # P0, P1, P2, P3
    generated_by_ai: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    status: Mapped[str] = mapped_column(String(50), nullable=False, default="Draft")  # Draft, Approved, Executing, Completed, Failed
    ...
    # Relationships
    steps: Mapped[list["AutomationStep"]] = relationship("AutomationStep", back_populates="runbook", cascade="all, delete-orphan")
    executions: Mapped[list["RunbookExecution"]] = relationship("RunbookExecution", back_populates="runbook", cascade="all, delete-orphan")


class AutomationStep(UUIDMixin, Base):
    __tablename__ = "automation_steps"

    runbook_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("runbooks.id", ondelete="CASCADE"), nullable=False)
...
class RunbookExecution(UUIDMixin, Base):
    ...
    runbook_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("runbooks.id", ondelete="CASCADE"), nullable=False)
    executed_by: Mapped[str] = mapped_column(String(255), nullable=False, default="CloudPulse AI Auto-Remediator")
