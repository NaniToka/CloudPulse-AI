...
class InfrastructureTwin(UUIDMixin, TimestampMixin, Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False, default="Primary Production Twin", index=True)
    status: Mapped[str] = mapped_column(String(50), nullable=False, default="synchronized")  # synchronized | simulating | degraded
...
class SimulationScenario(UUIDMixin, TimestampMixin, Base):
    ...
    category: Mapped[str] = mapped_column(String(100), nullable=False, default="Infrastructure")  # Infrastructure | Database | Network | Security
    failure_type: Mapped[str] = mapped_column(String(100), nullable=False)  # node_failure | pod_crash | redis_outage | region_failure | traffic_surge_400 | db_latency
    target_resource: Mapped[str] = mapped_column(String(255), nullable=False)
    description: Mapped[str] = mapped_column(Text, nullable=False)
    parameters: Mapped[dict] = mapped_column(JSON, default=dict)  # duration_min, surge_percent, latency_multiplier, etc.
    severity: Mapped[str] = mapped_column(String(20), nullable=False, default="HIGH")

    # Relationships
    twin: Mapped["InfrastructureTwin"] = relationship("InfrastructureTwin", back_populates="scenarios")
    executions: Mapped[list["SimulationExecution"]] = relationship(
...
class SimulationExecution(UUIDMixin, TimestampMixin, Base):
    ...
    confidence_score: Mapped[float] = mapped_column(Float, nullable=False, default=0.92)  # 0.0 - 1.0
    ...
    affected_services: Mapped[list] = mapped_column(JSON, default=list)  # ["checkout-svc", "auth-api", "postgres-primary"]
    blast_radius: Mapped[dict] = mapped_column(JSON, default=dict)  # { direct_impact: [], cascade_impact: [] }
    predicted_timeline: Mapped[list] = mapped_column(JSON, default=list)  # minute-by-minute failure progression
    recovery_steps: Mapped[list] = mapped_column(JSON, default=list)  # step-by-step mitigation actions

    started_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    completed_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)

    # Relationships
    twin: Mapped["InfrastructureTwin"] = relationship("InfrastructureTwin", back_populates="executions")
    scenario: Mapped["SimulationScenario"] = relationship("SimulationScenario", back_populates="executions")
...
class WhatIfQuery(UUIDMixin, TimestampMixin, Base):
    ...
    financial_risk_estimate: Mapped[str] = mapped_column(String(100), nullable=False, default="$12,000 / hr")
