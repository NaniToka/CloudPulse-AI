...
import type { GovernanceRemediationItem } from "@/types/governance";
    ...
    <div className="p-5 rounded-xl border border-white/10 bg-bg-elevated/40 backdrop-blur-md space-y-4 font-mono">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-foreground">Actionable Governance Remediation Plan</h3>
        </div>
        ...
        {onTriggerAnalysis && (
          ...
          >
            ...
            {isAnalyzing ? "Analyzing Posture..." : "Run AI Governance Analysis"}
          </button>
        )}
      </div>
          ...
          <div key={rec.id} className="p-4 rounded-lg border border-white/5 bg-black/40 space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-foreground text-xs">{rec.resource} ({rec.category})</span>
              </div>
            ...
            <div className="pt-1 flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-t border-white/5 text-xs text-brand-blue">
              ...
              <div className="flex items-center gap-3 shrink-0">
                <span className="text-slate-300 text-[11px]">Effort: {rec.estimated_effort}</span>
