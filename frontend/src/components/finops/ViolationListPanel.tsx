...
import { AlertCircle, CheckCircle, ShieldAlert, Clock, ArrowRight, Loader2 } from "lucide-react";
...
interface Props {
  ...
  onRequestRemediation: (viol: CostViolation) => Promise<void> | void;
}
...
export const ViolationListPanel: React.FC<Props> = ({
  ...
  const handleRemediateClick = async (viol: CostViolation) => {
    try {
      setRemediatingId(viol.id);
      await onRequestRemediation(viol);
    } finally {
  ...
  return (
    <div className="bg-slate-900/70 border border-slate-800/80 rounded-2xl p-6 shadow-2xl backdrop-blur-xl space-y-4 relative overflow-hidden">
      <div className="flex items-center justify-between border-b border-slate-800/80 pb-4">
        <div>
          <h3 className="text-base font-bold text-slate-100 tracking-tight">Policy Violation Center</h3>
      ...
      {loading ? (
        <div className="space-y-3">
          <div className="h-20 bg-slate-800/60 rounded-xl animate-pulse"></div>
          <div className="h-20 bg-slate-800/60 rounded-xl animate-pulse"></div>
        </div>
      ) : violations.length === 0 ? (
        ...
        <div className="space-y-3.5 max-h-[540px] overflow-y-auto pr-1">
          {violations.map((viol) => {
            const isRemediating = remediatingId === viol.id;

            return (
              <div
                key={viol.id}
                className="p-4 bg-slate-950/80 border border-slate-800/80 hover:border-slate-700/80 rounded-xl transition-all space-y-3 relative overflow-hidden"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className="font-bold text-sm text-slate-100 truncate" title={viol.policy_name}>
                        {viol.policy_name}
                      </span>
                      {getStatusBadge(viol.status)}
                    </div>
                    <p className="text-xs text-slate-300 leading-relaxed font-sans">{viol.explanation}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="text-xs font-bold text-rose-400 block">+${viol.difference.toLocaleString()} Over</span>
                    <span className="text-[10px] text-slate-400 font-mono">Actual: ${viol.actual_value.toLocaleString()}</span>
                  </div>
                </div>

                <div className="p-3 bg-slate-900/90 rounded-lg border border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                  <div className="space-y-0.5 min-w-0 flex-1">
                    <span className="text-[10px] uppercase font-bold text-indigo-400 block tracking-wider">Recommended Action</span>
                    <p className="text-slate-300 text-xs font-sans leading-snug">{viol.recommended_action}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                    <select
                      value={viol.status}
                      onChange={(e) => onUpdateStatus(viol.id, e.target.value)}
                      className="bg-slate-950 border border-slate-800 text-slate-300 rounded-lg px-2.5 py-1 text-[11px] font-mono focus:outline-none focus:border-indigo-500"
                    >
                    ...
                    <button
                      onClick={() => handleRemediateClick(viol)}
                      disabled={isRemediating}
                      className={cn(
                        "px-3 py-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-[11px] font-bold font-mono transition-all flex items-center gap-1.5 shadow-md shadow-indigo-600/25 disabled:opacity-50",
                        isRemediating && "cursor-wait"
                      )}
                    >
                      {isRemediating ? (
                        <>
